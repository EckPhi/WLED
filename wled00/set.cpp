...
void handleSettingsSet(AsyncWebServerRequest *request, byte subPage)
{
  ...
  if (subPage == 4)
  {
    ...
    if (t >= DMX_MODE_DISABLED && t <= DMX_MODE_MULTIPLE_RGBW)
      DMXMode = t;
  ...
  if (subPage != 6 || !doReboot)
    serializeConfig(); //do not save if factory reset
  if (subPage == 2)
  {
...
bool handleSet(AsyncWebServerRequest *request, const String &req, bool apply)
{
  ...
  if (pos > 0)
    strip.getSegment(selectedSeg).setOption(SEG_OPTION_REVERSED, req.charAt(pos + 3) != '0');

  //Segment reverse
  pos = req.indexOf(F("MI="));
  if (pos > 0)
    strip.getSegment(selectedSeg).setOption(SEG_OPTION_MIRROR, req.charAt(pos + 3) != '0');
