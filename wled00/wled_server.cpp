...
String dmxProcessor(const String &var)
{
  ...
  if (var == "DMXVARS")
  {
    ...
    mapJS += "0];";
    mapJS += "DP=" + String(DMXDirectionPin) + ";\n";
...
void serveSettings(AsyncWebServerRequest *request, bool post)
{
  ...
  { //settings/set POST request, saving
    ...
    if (!s2[0])
      strcpy_P(s2, PSTR("Redirecting..."));

    if (!doReboot)
      serveMessage(request, 200, s, s2, (subPage == 1 || subPage == 6) ? 129 : 1);
    if (subPage == 6)
      doReboot = true;
  ...
  case 6:
    ...
    break;
  case 7:
    request->send_P(200, "text/html", PAGE_settings_dmx, settingsProcessor);
    break;
  case 255:
    request->send_P(200, "text/html", PAGE_welcome);
    break;
  default:
