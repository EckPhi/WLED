...
void URL_response(AsyncWebServerRequest *request)
{
  ...
  if (request != nullptr)
    request->send(200, "text/html", obuf);
}
...
void getSettingsJS(byte subPage, char *dest)
{
  ...
  if (subPage == 4)
  {
    ...
    case HUE_ERROR_ACTIVE:
      strcpy(hueErrorString, (char *)F("Active"));
      break;
    case HUE_ERROR_UNAUTHORIZED:
      strcpy(hueErrorString, (char *)F("Unauthorized"));
      break;
    case HUE_ERROR_LIGHTID:
      strcpy(hueErrorString, (char *)F("Invalid light ID"));
      break;
    case HUE_ERROR_PUSHLINK:
      strcpy(hueErrorString, (char *)F("Link button not pressed"));
      break;
    case HUE_ERROR_JSON_PARSING:
      strcpy(hueErrorString, (char *)F("JSON parsing error"));
      break;
    case HUE_ERROR_TIMEOUT:
      strcpy(hueErrorString, (char *)F("Timeout"));
      break;
    default:
...
#ifndef ESP8266
    sappend('v', SET_F("DP"), DMXDirectionPin);
    sappend('v', SET_F("OP"), DMXSerialOutputPin);
#endif
